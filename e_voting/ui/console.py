...
from e_voting.constants import HEADER_BOX_WIDTH, MENU_NUMBER_WIDTH
...
def header(title, theme_color):
    ...
    top = f"  {theme_color}{'═' * HEADER_BOX_WIDTH}{RESET}"
    mid = (f"  {theme_color}{BOLD} {title.center(HEADER_BOX_WIDTH - 2)} "
           f"{RESET}{theme_color} {RESET}")
    bot = f"  {theme_color}{'═' * HEADER_BOX_WIDTH}{RESET}"
    print(top)
    print(mid)
    print(bot)
...
def menu_item(number, text, color):
    ...
    print(f"  {color}{BOLD}{number:>{MENU_NUMBER_WIDTH}}.{RESET}  {text}")
